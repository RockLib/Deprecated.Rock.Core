...
namespace Rock.Core.UnitTests.Extensions
{
    ...
    public class StringExtensionsTest
    {
        ...
        [Test]
        public void String_Extension_Can_Deserialize_JsonToObject()
        {
            string json = @"{""key1"":""value1"",""key2"":""value2""}";

            var o = json.FromJson(typeof(JsonTestClass));
            var value = o as JsonTestClass;
        ...
        [TestCase("<Xml><Element>value</Element></Xml>", "value")]
        public void String_Extension_Can_Deserialize_XmlToObject(string xmlText, string elementValue)
        {
            var o = xmlText.FromXml(typeof(Xml));
            var xml = o as Xml;
            Assert.IsNotNull(xml);
            Assert.AreEqual(elementValue, xml.Element);
        }
